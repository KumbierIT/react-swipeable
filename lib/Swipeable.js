...
function selectChangedTouchesById(changedTouches, id) {
  ...
  return Array.from(changedTouches).filter(function (touch) {
...
function getPosition(e) {
  var enableMultiTouch = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;

  if (enableMultiTouch) {
    return getPositionTargetTouches(e);
  }

  return 'touches' in e ? { x: e.touches[0].clientX, y: e.touches[0].clientY, identifier: e.touches[0].identifier } : { x: e.clientX, y: e.clientY, identifier: -1 };
}
...
function rotateByAngle(pos, angle) {
  ...
  return { x: rotatedX, y: rotatedY, identifier: identifier };
}

function calculatePos(e, state) {
  var _rotateByAngle = rotateByAngle(getMovingPosition(e, state.id), state.rotationAngle),
      x = _rotateByAngle.x,
      y = _rotateByAngle.y;
...
var Swipeable = function (_React$Component) {
  ...
  Swipeable.prototype.eventStart = function eventStart(e) {
    var _props = this.props,
        ...
        enableMultiTouch = _props.enableMultiTouch;

    if (e.touches && e.touches.length > 1 && !enableMultiTouch) return;

    var _rotateByAngle2 = rotateByAngle(getPosition(e, enableMultiTouch), rotationAngle),
        x = _rotateByAngle2.x,
        y = _rotateByAngle2.y,
        identifier = _rotateByAngle2.identifier;

    if (this.props.stopPropagation) e.stopPropagation();

    this.swipeable = { start: Date.now(), x: x, y: y, swiping: false, rotationAngle: rotationAngle, id: identifier };
  };
  ...
  Swipeable.prototype.eventMove = function eventMove(e) {
    var _props2 = this.props,
        ...
        preventDefaultTouchmoveEvent = _props2.preventDefaultTouchmoveEvent,
        enableMultiTouch = _props2.enableMultiTouch;


    if (!this.swipeable.x || !this.swipeable.y || e.touches && e.touches.length > 1 && !enableMultiTouch) {
      return;
    }
  ...
  Swipeable.prototype.eventEnd = function eventEnd(e) {
    var _props3 = this.props,
        stopPropagation = _props3.stopPropagation,
